...
var wattstrat = angular.module('wattstrat', []);
...
wattstrat.service('wsUtils', function($http, $templateCache){
    /**
    Transform an array of object into a dict, using one attr for the dict key and the element as a value
    ...
    */
    this.array_to_dict = function(array, key){
        var dict = {};
    ...
    /**
    Prefetch a template file for faster display when needed
    */
    this.prefetch_template = function(url){
        //prevent the prefetching if the template is already in the cache
        if ($templateCache.get(url))
            return; 
        
        $http.get(url).success(function (t) {
            $templateCache.put(url, t);
        });
    };
});

wattstrat.service('messages', function($http){
    this.state = {
        saved: false,
        error: false
    };
    this.show = function(name, text){
        if( !text ){
            text=true;
        }
...
wattstrat.service('locale', function(){
    this.country = 'france'; // We work only with france for now
});

wattstrat.service('geocodes', Geocodes.factory);
wattstrat.service('resultvariables', ResultVariables.factory);
wattstrat.service('parametervariables', ParameterVariables.factory);

wattstrat.directive('wsDate', function(){
    return {
        templateUrl: '/static/angular/inputs/date.html',
        transclude: true,
        /* Bind the model for the date */
        scope:{
            required: '=',
            dateModel: '=',
            minDate: '=',
        ...
        link: function(scope, element, attrs) {
            scope.opened = false; // Manage popup opening
...
wattstrat.directive('wsDateRange', function(){
    return {
        templateUrl: '/static/angular/inputs/date_range.html',
        scope:{
            required: '=',
            startDateModel: '=',
            endDateModel: '=',
        ...
        link: function(scope, element, attrs) {
            function validate(startDate, endDate) {
                ...
                var controller = angular.element(input).controller('ngModel');
                if( !controller ){
                    return;
                }
                if(!startDate || !endDate){
                    controller.$setValidity('dateOrder', true);
                }
                else{
                    controller.$setValidity('dateOrder', startDate < endDate);
                }
                
            }
            
            scope.$watchGroup(['startDateModel', 'endDateModel'], function(dates){
                validate(dates[0], dates[1]);
            });
        } 
...
function BaseSection($http, messages, wsUtils, parametersUrl) {
    var section = this;
    this.definition = {};
    this.sectionBreadcrumb = [];
    
    this.categoriesDict = {};
    this.subcategoriesDict = {};
    
    this.setParametersDefinition = function(data){
        section.definition = data.definition;
        section.sectionBreadcrumb = data.section_breadcrumb;
        
        if( section.isCategorized() ){
            section.categoriesDict = wsUtils.array_to_dict(data.definition.categories, 'name'); 
            angular.forEach(section.categoriesDict, function(category){
                category.subcategoriesDict = wsUtils.array_to_dict(category.subcategories, 'name'); 
            });
        }
    };
    ...
    this.dataPromise = $http.get(parametersUrl)
        .success(function(data){
            section.setParametersDefinition(data);
        })
        .error(function(){
            messages.show('error');
        });
...
wattstrat.directive('wsValidate', function($timeout){
    return {
        restrict: 'A',
        scope: true,
        link: function(scope, element, attrs) {
            var form = scope[attrs.wsValidate];
            ...
            scope.$watch(function() {
                ...
                inputs.each(function(i, input){
                    ...
                    if(controller.$invalid && (controller.$touched || form.$submitted)){
                        validState.isInvalid = true;
                    }
                ...
                // A parameter is valid if any control has been touched and no control is invalid
                validState.isValid = ( anyTouched || form.$submitted ) && !anyInvalid;
                return validState;
            }, function(validState) {
                element.toggleClass('has-error', validState.isInvalid);
                element.toggleClass('has-success', validState.isValid);
                scope.validState = validState;
            },
            true);
        } 
    };
...
wattstrat.service('userinfo', function($http, userinfoUrl){
    var userinfo = this;
    this.infos = null;
    
    this.dataPromise = $http.get(userinfoUrl)
        .success(function(data){
            userinfo.infos = data;
        })
        .error(function(){
